...
import { SocialUserModel } from "../../model/index";
...
export const getUser = async (
  ...
  const users = await SocialUserModel.findAll();
  res.status(200).json({
    success: true,
...
export const createUser = async (
  ...
  const newUser = await SocialUserModel.create(userData);
  res.status(201).json({
    success: true,
...
export const updateUser = async (
  ...
  const [count, rows] = await SocialUserModel.update(dataForUpdate, {
    where: { id: userId },
    returning: true,
  });
  res.status(200).json({
...
export const deleteUser = async (
  ...
  const user = await SocialUserModel.findByPk(userId);
  if (user) {
  }
  const deletedUser = await SocialUserModel.destroy({
    where: { id: userId },
  });
  res.status(200).json({
    success: true,
