import { MiniEcomProductModel } from "../../model/index";
...
export const getProduct = async (
  ...
  const { count, rows } = await MiniEcomProductModel.findAndCountAll({
    limit: limit,
    offset: offset,
...
export const createProduct = async (
  ...
  const newProduct = await MiniEcomProductModel.create(productData);
  res.status(200).json({
    success: true,
...
export const updateProduct = async (
  ...
  const [count, rows] = await MiniEcomProductModel.update(dataForUpdate, {
    where: { id: productId },
    returning: true,
  });
  res.status(200).json({
...
export const deleteProduct = async (
  ...
  const product = await MiniEcomProductModel.findByPk(productId);

  const deletedProduct = await MiniEcomProductModel.destroy({
    where: { id: productId },
  });
  res.status(200).json({
    success: true,
