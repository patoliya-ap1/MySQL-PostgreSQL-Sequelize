...
import { EcomUserModel } from "../../model/index";
...
export const getUser = async (
  ...
  const users = await EcomUserModel.findAll();
  res.status(200).json({
    success: true,
...
export const createUser = async (
  ...
  const newUser = await EcomUserModel.create(userData);
  res.status(200).json({
    success: true,
...
export const updateUser = async (
  ...
  const [count, rows] = await EcomUserModel.update(dataForUpdate, {
    where: { id: userId },
    returning: true,
  });
  res.status(200).json({
...
export const deleteUser = async (
  ...
  const user = await EcomUserModel.findByPk(userId);
  if (user) {
  }
  const deletedUser = await EcomUserModel.destroy({
    where: { id: userId },
  });
  res.status(200).json({
    success: true,
